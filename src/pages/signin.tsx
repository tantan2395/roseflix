		...
		if (canProceed) {
			mainHttp
				...
				.then(({ authorization, data }: any) => {
					setIsLoading(false);
					localStorage.setItem('SVFlix-auth', authorization);
					localStorage.setItem('SVFlix-user', JSON.stringify(data.userDetails));
	...
	return (
		<Scrollbar noDefaultStyles className="main-scrollbar" onScroll={({ scrollTop }: any) => handleOnScroll(scrollTop)}>
			...
			<Form>
				...
				<Form.Text>
					New to SVFlix? <Form.Link to={ROUTES.SIGNUP.path}>Sign up now</Form.Link>.
				</Form.Text>
